...
ComplexEigenPairs IRAM(const M& M_, cublasHandle_t& handle, cusolverDnHandle_t& solver_handle, const HostPrecision& tol = default_tol) {
    ...
    const size_t num_loops = std::ceil(A / C);
    for (int i = 0; i < num_loops; i++) {
        auto start_iter = std::chrono::high_resolution_clock::now();
        if (i == 0) {KrylovIterInternal<M, DS, N, N, B>(Q, H_tilde, m, M_, d_M, d_y, d_result, d_evecs, d_h, norms, ROWS, handle, matnorm);}
        // else {KrylovIterInternal<M, DS, N, N, B, C>(Q, H_tilde, m, M_, d_M, d_y, d_result, d_evecs, d_h, norms, ROWS, handle, matnorm, tol, true);}
        assert(isOrthonormal<OM>(Q.block(0,0,N,B)));
    ...
    return {Q.leftCols(B), H_square, B};
